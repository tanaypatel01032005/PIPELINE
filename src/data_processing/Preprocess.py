"""
Preprocess.py - Preprocessing & Cleaning Stage
Cleans and prepares the raw merged commodity-currency dataset.

Input  : data/processed/RAW Merged Data.csv
...
# Columns that should use Kalman Smoother (Standardized Names)
KALMAN_COLUMNS = [
    "Corn",
    "Heating_Oil",
    "Lean_Hogs",
...
for col in value_cols:
    ...
    if col in KALMAN_COLUMNS:
        ...
        # Save first and last valid index of the original data to preserve leading/trailing NaNs
        ...
        try:
            ...
            # Re-mask leading/trailing NaNs to prevent look-ahead leakage
            if first_valid_idx is not None:
                df.loc[df.index < first_valid_idx, col] = np.nan
            if last_valid_idx is not None:
                df.loc[df.index > last_valid_idx, col] = np.nan
